...
{
	if (((p->f->p == 1 && p->f->z == 0 && p->m == 0) ||
		(p->f->p == 1 && p->f->z == 1 && p->m == 0 && (p->f->m
		== 1 || (p->w >= p->precision && p->hp != 0)))) && p->t != 'u')
		ft_putchar('+');
	if (p->t == 'p' || (p->t == 'x' && p->f->o == 1 && *((unsigned int*)d)
		!= 0 && (p->f->z == 0 || (p->f->z == 1 && (p->f->m == 1 || p->w
		<= p->precision || (p->precision < p->w && p->hp != 0))))))
		ft_putstr("0x");
	if (p->t == 'X' && p->f->o == 1 && *((unsigned int*)d) != 0 &&
		(p->f->z == 0 || (p->f->z == 1 && (p->f->m == 1 || p->w
		<= p->precision || (p->precision < p->w && p->hp != 0)))))
		ft_putstr("0X");
