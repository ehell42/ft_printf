...
void	look_at_w(unsigned int len, unsigned int width, t_print *p, void *d)
{
	char symbol;

	symbol = ' ';
	if (p->flag->plus == 0 && p->flag->probel == 1 && (p->type == 'f'
	|| p->type == 'F' || p->type == 'd' || p->type == 'i') && p->minus == 0)
		putlesswdth(' ', &width);
	if (p->flag->minus == 1)
		outputdata(d, p);
	else if (p->flag->zero == 1)
	{
		if (((p->precision == 0 || p->minus == 1) && p->haveprecision != 1)
		|| (p->haveprecision == 1 && p->precision < 0) || p->type == 'f')
			symbol = '0';
		if ((p->minus == 1 && p->haveprecision != 1) || l(p))
		{
			if (p->type != 'f')
				itis(d, p);
			ft_putchar('-');
